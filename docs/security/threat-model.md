...
- **Impact:** Full session takeover for the cookie's lifetime (bounded by idle/absolute timeout, once those values are fixed — see `open-decisions.md` decisions 4–5).
...
- **Entry point:** Ordinary login, if concurrent sessions are permitted without bound
- **Trust boundary crossed:** Session-issuance boundary
- **Preconditions:** `open-decisions.md` decision 18 (concurrent sessions per human identity) resolves toward unrestricted or loosely-bounded concurrency, or is left unresolved and defaults to whatever an implementer happens to build.
- **Attack scenario:** An attacker who has obtained a valid credential (phished, stolen) logs in alongside the legitimate user without ever needing to displace or be noticed by the legitimate session, if unlimited concurrent sessions are allowed and no device-visibility capability exists for the legitimate user to notice the extra session.
- **Impact:** High — extends the *duration* and *stealth* of a credential compromise materially, compared to a single-session model where a new attacker login would evict the legitimate user (a noisy, self-alerting signal).
- **Likelihood:** Medium, **and explicitly unresolved as a design question** — this is not a case of "the design specifies a control and we rate its likely effectiveness"; per `open-decisions.md` decision 18, this is genuinely "Open, unresolved... surfaced by this design phase's own security architecture review," with three named alternatives and no recommendation offered as a settled choice. **Impact:** High (taken as the upper bound of the prior "Medium-High" hedge, per §7.0's no-hedge rule). **Inherent risk:** High (base matrix: Medium + High = High, §7.0), **and this document explicitly declines to rate a residual risk against a control that has not been chosen.**
- **Required controls:** Whichever of the three alternatives (single session / bounded multiple / unrestricted) decision 18 eventually resolves to, plus its interacting logout-all, device-visibility, and account-recovery behavior — none of which is currently specified regardless of which alternative is chosen.
- **Current control status:** Not applicable today.
- **Residual risk — current v0.1:** None.
- **Residual risk if Proposed implemented and gate-verified:** **Cannot be assessed** — this is not a gate-verification gap (like several threats above, where the design is sound and only implementation/testing remains); it is a **design gap**: no chosen policy exists yet to verify against. This document records it as **unresolved by design**, not merely unimplemented.
- **Detection/audit expectations:** Session-lifecycle audit events would capture each individual session's creation, but no cross-session correlation ("this identity now has N concurrent sessions") is specified as a distinct audit signal.
- **References:** `open-decisions.md` decision 18 (explicitly unresolved); Pass 5, Pass 18.
...
| TM-E04 | Concurrent-session abuse | E | A1,A2 | High | Not applicable | None | Cannot be assessed — no policy chosen | UNR | Decision 18 | — |
...
**Partially mitigated (PM in §9.1):** TM-T02, TM-T05, TM-R01, TM-R02, TM-I08, TM-D01, TM-E03, TM-E07.

**Unresolved (UNR in §9.1):** TM-S11, TM-T03, TM-T04, TM-R04, TM-I02, TM-I03, TM-I07, TM-I09, TM-D02, TM-D03, TM-E04, TM-E05.
...
| Session-store selection | Decision 3 | Medium-High | Open, recommended | Whoever implements Pass 4, confirmed against ADR-0002's existing decision | TM-I08 |
| Idle timeout | Decision 4 | Medium-High | Open, no numeric value fixed | A product/security stakeholder informed by real analyst workflow | TM-S06 |
| Absolute timeout | Decision 5 | Medium-High | Open, no numeric value fixed | Same as idle timeout | TM-S06 |
...
| Concurrent sessions | Decision 18 | Medium-High | Open, unresolved | A product/security stakeholder informed by real analyst multi-device usage patterns | TM-E04 (explicitly unresolved, §7) |
