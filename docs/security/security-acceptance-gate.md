...
| I6 | **Session rotation.** Rotation occurs on login, on privilege/role change, and periodically during a long-lived session, per `identity-and-session-architecture.md`. `open-decisions.md` decision 18 (concurrent sessions per human identity) is resolved — a human identity may hold at most 3 active sessions; rotation's interaction with a user's other, up-to-2 concurrent sessions is part of this item, not a separate one. Also covers the resolved at-cap login behavior: a login attempt at the 3-session limit must not create a session until an existing one is explicitly selected and revoked (`identity-and-session-architecture.md` sequence diagram 6). | Integration tests for each of the three rotation triggers, confirming the prior identifier is invalidated; an integration test for the at-cap login flow, confirming no session is created without an explicit revocation selection. |
| I7 | **Session revocation.** Logout, self-service revocation of one session, self-service logout-all, and administrator-forced revocation each take effect on the very next request, with no propagation delay or caching window. Also covers `open-decisions.md` decision 18's security-triggered global revocation (password change, disablement, compromise handling) and its self-service view/revoke-one/logout-all surface, which must authorize strictly by ownership of the affected identity, never allowing one identity to view or revoke another's session. | Integration test: a session revoked mid-test (by any of the paths above) is rejected on the immediately following request; an integration test confirming a cross-identity self-service revocation/view attempt is rejected. |
| I8 | **Idle and absolute expiry.** Both are enforced by re-checking the persisted session row on every request, against the resolved values (`open-decisions.md` decisions 4–5): 20-minute idle timeout, refreshed only by genuine user interaction, never by background polling, an automatic refresh, WebSocket traffic, or a heartbeat; 8-hour absolute timeout, never extended by activity, rotation, or token renewal. | Integration tests at the idle-timeout and absolute-timeout boundaries (just inside and just outside each window); an integration test confirming a background/polling request does not refresh the idle timer. |
